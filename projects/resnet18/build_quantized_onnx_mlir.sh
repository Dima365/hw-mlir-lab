...
if ! conda env list | awk '{print $1}' | grep -qx "$ENV_NAME"; then
  ...
  echo "[1/5] Conda env already exists: $ENV_NAME"
fi
...
echo "[3/5] Pulling ONNX-MLIR Docker image"
docker pull "$DOCKER_IMAGE"

echo "[4/5] Exporting quantized ResNet18 to ONNX"
conda run -n "$ENV_NAME" python resnet18_to_mlir.py

echo "[5/5] Converting ONNX to ONNX dialect MLIR"
docker run --rm \
  -v "$PWD":/work \
  "$DOCKER_IMAGE" \
  "$ONNX_MLIR_BIN" \
