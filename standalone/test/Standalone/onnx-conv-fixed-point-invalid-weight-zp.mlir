...
module {
  func.func @nonzero_weight_zero_point(%input_q: tensor<1x1x2x2xui8>) {
    ...
    %ws = "onnx.Constant"() {value = dense<0.25> : tensor<1xf32>}
        : () -> tensor<1xf32>
    %wz = "onnx.Constant"() {value = dense<1> : tensor<1xi8>}
        : () -> tensor<1xi8>
...
// CHECK: weight zero point at channel 0 must be zero
