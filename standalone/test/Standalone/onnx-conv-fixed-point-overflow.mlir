...
module {
  func.func @coefficient_overflow(%input_q: tensor<1x1x2x2xui8>) {
    %as = "onnx.Constant"() {value = dense<1.0> : tensor<f32>}
        : () -> tensor<f32>
    ...
    %wq = "onnx.Constant"() {value = dense<0> : tensor<1x1x1x1xi8>}
        : () -> tensor<1x1x1x1xi8>
    %ws = "onnx.Constant"() {value = dense<1.0> : tensor<1xf32>}
        : () -> tensor<1xf32>
    ...
    %bq = "onnx.Constant"() {value = dense<0> : tensor<1xi32>}
        : () -> tensor<1xi32>
    %bs = "onnx.Constant"() {value = dense<1.0> : tensor<1xf32>}
        : () -> tensor<1xf32>
    ...
    %conv = "onnx.Conv"(%input, %weight, %bias)
        : (tensor<1x1x2x2xf32>, tensor<1x1x1x1xf32>, tensor<1xf32>)
          -> tensor<1x1x2x2xf32>
    %os = "onnx.Constant"() {value = dense<1.0e-10> : tensor<f32>}
        : () -> tensor<f32>
...
// CHECK: fixed-point coefficient at channel 0: real multiplier does not fit signed i32
