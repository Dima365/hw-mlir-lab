...
module {
  func.func @bad_weight_scale_count(%input_q: tensor<1x1x2x2xui8>) {
    ...
    %wq = "onnx.Constant"() {value = dense<0> : tensor<1x1x1x1xi8>}
        : () -> tensor<1x1x1x1xi8>
    %ws = "onnx.Constant"() {value = dense<[0.25, 0.125]> : tensor<2xf32>}
        : () -> tensor<2xf32>
    %wz = "onnx.Constant"() {value = dense<0> : tensor<1xi8>}
        : () -> tensor<1xi8>
    %weight = "onnx.DequantizeLinear"(%wq, %ws, %wz) {axis = 0 : si64}
        : (tensor<1x1x1x1xi8>, tensor<2xf32>, tensor<1xi8>)
          -> tensor<1x1x1x1xf32>

    %bq = "onnx.Constant"() {value = dense<0> : tensor<1xi32>}
        : () -> tensor<1xi32>
    %bs = "onnx.Constant"() {value = dense<0.125> : tensor<1xf32>}
        : () -> tensor<1xf32>
...
// CHECK: weight scale must be tensor<Coutxf32>
